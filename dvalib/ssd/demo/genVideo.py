...
def bboxes_draw_on_img_pil(img, classes, scores, bboxes, colors, thickness=2):
    ...
    # img.save(r'pil.jpg')
    # cv2.imwrite('opencv4.jpg', np.asanyarray(img))
...
def video():
    video_path = r'street.mp4'
    capture = cv2.VideoCapture(video_path)
    if capture.isOpened:
        print( 'read successful\n')
        statu,frame=capture.read()

        writer=cv2.VideoWriter('street_detect.mp4',fourcc=cv2.cv.CV_FOURCC('M', 'P', 'E', 'G'),frameSize=(544,408),fps=25)
        if writer.isOpened:
            print 'da kai xie le '
        else:print 'no'
        count=1
        print frame.shape[:2]
        total=capture.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT)
        while(statu):
            if count%1==0:
                rclasses, rscores, rbboxes = process_image(frame)
                img = bboxes_draw_on_img_pil(frame, rclasses, rscores, rbboxes, colors_plasma)
                writer.write(img)
            statu,frame=capture.read()
            count+=1
            print ('running %d/%d'%(count,total))
            # statu=False
        # writer.release()
    else :
        print 'no'
